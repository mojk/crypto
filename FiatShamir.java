		...
		**/
		for(int i = 0; i < runs.length; i++) {
			for(int j = i+1; j < runs.length; j++) {
				if(runs[i].R.equals(runs[j].R)) { // Comparing R for each run of the protocol
					
					if(runs[i].c == 0) { // we want to know when c = 0, since a = R * S^0 mod n => a = R * 1 mod n
						BigInteger inv = runs[i].s.modInverse(N); // a = R * S^0 mod N, a^-1 = 1/R * 1 mod N
						BigInteger secret = inv.multiply(runs[j].s).mod(N); // a^-1 * a' = 1/R * 1 mod N * R * S^1 mod N => S
